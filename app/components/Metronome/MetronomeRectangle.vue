...
<template>
    <MetronomeBase v-model="metronome" :text-color="textColor(tick)">
        <template #default>
            <div :class="['rounded-3xl w-full h-full flex items-center justify-center font-bold text-3xl shadow-[0_0_50px_-5px_inset_#0001]', textColor(tick)]"
                :style="{ backgroundColor: backgroundColor(tick) }">
                {{ metronome.title }}
            </div>
        </template>
        <template #floating-settings>
            <MetronomeQuickSettingButton tooltip="Change color" :popover-props="{ content: { align: 'start' } }">
                <UIcon name="i-ic-outline-palette" mode="svg" class="text-2xl transition-none" data-palette-icon />
                <template #popover>
                    <div class="p-2">
                        <UColorPicker v-model="metronome.style.colorBackground" format="hex" size="xs" />
                    </div>
                </template>
