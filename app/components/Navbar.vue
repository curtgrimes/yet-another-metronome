...
<template>
  <div class="fixed inset-x-0">
    <UTooltip text="Edit" :delay-duration="0">
      <UButton v-if="minimizeNavbar" :style="{ viewTransitionName: 'navbar-minimize' }" size="lg"
        class="absolute top-4 right-4 aspect-square rounded-full text-xl p-2" variant="soft" @click="mode = 'edit'">
        <UIcon name="i-mingcute-settings-7-line" />
      </UButton>
    </UTooltip>
    <UCard v-if="!minimizeNavbar" variant="soft" class="max-w-4xl mx-auto mt-4 backdrop-blur-2xl"
      :style="{ viewTransitionName: 'navbar-minimize' }">
      <div class="flex gap-4">
        <UPopover mode="hover">
          <UButton size="lg" icon="i-mingcute-add-line" label="Add Metronome" variant="soft" />

          <template #content>
            Hello world
          </template>
        </UPopover>
        <UButton size="lg" icon="i-mingcute-layout-3-line" label="Edit Layout" variant="ghost" />
        <UPopover mode="hover" :dismissible="false">
          <UTabs v-model="mode" :content="false" :items="editPerformTabItems" class="ml-auto">
          ...
          <template #content>
            <div class="p-4 flex flex-col gap-4 text-sm">
              <div v-for="(item, index) in editPerformTabItems" :key="index">
                <h3 class="font-bold flex items-center gap-2">
                  <UIcon :name="item.icon!" class="w-4" /> {{ item.label }}
                </h3>
                <p class="ml-6">
                  {{ item.description }}
                </p>
